...
# Create a final QQ plot for the transformed response variable 'perfo'
plt.figure(figsize=(8, 6))
stats.probplot(transformed_data['perfo'], dist="norm", plot=plt)
...
plt.figure(figsize=(8, 6))
plt.hist(transformed_data['perfo'], bins=20, edgecolor='black', alpha=0.7)
...
# Refit the model to ensure 'model' is properly defined
model = sm.OLS(y_transformed, X_transformed_with_const).fit()
...
bonferroni_p_values = p_values * len(y_transformed)
...
# Check for observations exceeding the Bonferroni significance threshold
influential_points = np.where(bonferroni_p_values < 0.05)[0]
influential_points_indices = influential_points.tolist()
