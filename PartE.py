...
plt.title("Residuals vs Fitted Values  2 model ")
...
plt.title("Residuals vs Fitted Values with U-Shaped Quadratic Curve 2 model")
plt.xlabel("Fitted Values")
plt.ylabel("Residuals")
plt.legend()
plt.grid(True)
...
for predictor in predictors:
    ...
    plt.plot(x_curve, y_curve, color='blue', label="Quadratic Curve (U-shaped)", linewidth=2)
    plt.axhline(y=0, color='red', linestyle='--', label="Zero Line")
    plt.title(f"Residuals vs {predictor} with U-Shaped Quadratic Curve 2 model")
...
plt.axhline(y=0, color='red', linestyle='--', label="Zero Line")
plt.title("Residuals vs Order 2 model ")
plt.xlabel("Order")
plt.ylabel("Residuals")
plt.grid(True)
plt.legend()
...
axes[0, 0].set_title("Cook's Distance 2 model ")
...
axes[0, 1].set_title('Standardized Residuals 2 model')
...
axes[1, 0].set_title('Hat Values (Leverage) 2 model')
...
axes[1, 1].set_title('Bonferroni p-values 2 model')
...
stats.probplot(transformed_data_clean_2model['perfo'], dist="norm", plot=plt)
plt.title("QQ Plot for Transformed Response Variable (perfo) 2 model ")
plt.grid(True)
plt.show()

# Plot a histogram for the transformed response variable 'perfo'
plt.figure(figsize=(8, 6))
plt.hist(transformed_data_clean_2model['perfo'], bins=15, edgecolor='black', alpha=0.7)
plt.title("Histogram of Transformed Response Variable (perfo) 2 model ")
...
print("\nP value seperately 2nd model\n", p_values)
print("\nSD seperately 2nd model\n", standard_errors)
...
for i, predictor in enumerate(predictors):
    ...
    axes[i].set_title(f"Effect Plot for {predictor} 2nd model")
...
plt.suptitle("\nEffect Plots for Predictors 2nd model\n", fontsize=16)
plt.show()

# Display ANOVA table and parameter estimates
print("\nAnova Table 2nd model\n",anova_results)
print("\nParameter Estimates 2nd model :\n")
print(params)
